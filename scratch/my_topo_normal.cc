...
uint32_t 	queueBytes 		  = 325e3;      // Number of bytes per queue port
uint32_t 	queueBytesToR		  = 225e3;
//TODO: different queue sizes for spines and switches
uint32_t 	initCwnd 		  = 10;         // TCP Initial Congestion Window
double 		minRto 			  = 10000e-6; 
uint32_t 	segmentSize 		  = 1460;
bool            LinkUtilization           = 0;
string          StatsFileName             = "PriorityQueue.txt";
double          interval                  = 0.01;

uint32_t	incastDegree		  = 26;
...
void Configure_Simulator(){
  ...
  NumberofIncastSenders = NumberofEndPointsIncast - 20;//incastDegree * ((int) (NumberofEndPointsIncast/incastDegree));
...
void Build_Topology()
{
  ...
  PointToPointHelper e2t;
  e2t.SetDeviceAttribute        ("DataRate",  StringValue (LinkSpeed));
  e2t.SetChannelAttribute       ("Delay",     StringValue (LinkDelay));
  Ptr<AttributeValue> v4 = Create<UintegerValue> (queueBytesToR);


  e2t.SetQueue  (queueType,
              n1, *v1,
              n2, *v4,
              n3, *v3);
  ...
  for (uint32_t i=0;i<NumberofEndPoints;i++){
    devices = e2t.Install (nEnd[i],nLeaf[i/MachinesPerRack]);
    interfaces = ipv4.Assign(devices);
    ipv4.NewNetwork();
  }
//connect incast aggregators to spine switches too.
 // for (uint32_t i=19;i<NumberofEndPoints;i+=20){
 //     for (uint32_t j=0;j<NumberofSpineSwitches-6;j++){
 //   	devices = p2p.Install (nEnd[i],nSpine[j]);
 //   	interfaces = ipv4.Assign(devices);
 //   	ipv4.NewNetwork();
 //   }
 // }
...
void Setup_Workload(){
  ...
  uint32_t min = 0.0;
  uint32_t max = MachinesPerRack-2;
...
//  NS_ASSERT((AppIdxIncast.size()+AppIdxIncastagg.size()) == NumberofEndPointsIncast);
  NS_ASSERT(AppIdxIncast.size() == NumberofIncastSenders);
...
//		  cout<<"ClientNodeIdx\t"<<ClientNodeIdx<<"\n";
 
    		Prev_Start[ClientNodeIdx] = Curr_Start;
      		SetupServerTraffic(nEnd[ServerNodeIdx],appPort,Curr_Start, 1);
      		SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start, 1);
		NumShortFlows++;
  //                        cout<<"ServerNodeIdx\t"<<ServerNodeIdx<<"\n";
	        ...
	        Curr_Start = Prev_Start[ServerNodeIdx] + Seconds (DelayRandomlyShort->GetValue());
                ...
                SetupClientTraffic(nEnd[ServerNodeIdx],nEnd[ClientNodeIdx],FlowSizeShort,appPort,Curr_Start, 1);
		NumShortFlows++;
	}
  }
...
//  NS_ASSERT(IncastAggregators.size() == NumberofIncastAggregators);
  ...
  for (uint32_t k=0;k < IncastRequestsPerNode; k++){
		...
		for(uint32_t j=0;j<ClientIdx.size();j++)
		{
		    ...
		    //    if (j==0){     
                      //  SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],IncastFlowSizeLong,appPort,Curr_Start,1);

			//	}
		//	else {
			SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start+Seconds (0.000080 + AppDelay()/1000000),1);
 		//		}
		}
        	NumIncastFlows++;
	}
...
//        if(dtqueue->Getqueuesize()!=0){
	  ...
	  //  std::cout<<"Queue:"<<"\t"<<dtqueue->m_bytesInQueue<<"\n";
