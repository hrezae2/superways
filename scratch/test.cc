...
double  AppDelay(){
  ...
  ApplicationDelay->SetAttribute ("Mean", DoubleValue (0.000010));
  ApplicationDelay->SetAttribute ("Bound", DoubleValue (0.000120));
...
void Setup_Workload(){
  ...
  for (uint32_t k=0;k < IncastRequestsPerNode; k++){
		...
		for(uint32_t j=0;j<ClientIdx.size();j++)
		{
			...
			SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start+Seconds (AppDelay()),1);
		}
        	NumIncastFlows++;
	}
  }
