...
void SetupServerTraffic (Ptr<Node> Nd, uint16_t appPort, Time startTime, bool IncastAgg) 
{
...
void Setup_Workload(){
  ...
  for (uint32_t k=0;k < ShortRequestsPerNode; k++){
  	...
  	for (uint32_t i=0;i<AppIdxShort.size(); i=i+2){
        	...
        	Map_Port[ServerNodeIdx].push_back(appPort);
      		...
      		SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start);
		NumShortFlows++;
                Curr_Start = Prev_Start[ServerNodeIdx] + Seconds (DelayRandomlyShort->GetValue());
                Prev_Start[ServerNodeIdx] = Curr_Start;
                SetupServerTraffic(nEnd[ClientNodeIdx],appPort,Curr_Start, false);
                SetupClientTraffic(nEnd[ServerNodeIdx],nEnd[ClientNodeIdx],FlowSizeShort,appPort,Curr_Start);
		NumShortFlows++;
	}
  }
 ...
 for(uint32_t i=0; i <IncastAggregators.size();i++){
  ...
  for (uint32_t k=0;k < IncastRequestsPerNode; k++){
		...
		//	SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start+Seconds (ApplicationDelay->GetValue()));
			SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start+Seconds (AppDelay()));
		}
        	NumIncastFlows++;
	}
  }
  ...
  for(uint32_t j = 0; j < rnodeip->GetNInterfaces(); j++) {
	...
	if (i == 7){
        if(dtqueue->Getqueuesize()!=0){
            std::cout<<"SwitchID: "<<i<<" Port: "<<j<<" BytesinQueue: "<<dtqueue->m_bytesInQueue<< " packetNum: "<<dtqueue->Getqueuesize() << " Time: "<< Simulator::Now ().GetMilliSeconds() <<"\n";
        }
      }
    }

  }
  Simulator::Schedule(Seconds(0.001),&GetQueue,q);
...
Simulator::Schedule(Seconds(0.001),&GetQueue,dtqueue);
    }
  }
