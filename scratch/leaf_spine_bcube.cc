...
// Parameters
double          d_LinkSpeed               = 1;
string          LinkSpeed                 = "1Gbps";
string          LinkDelay                 = "15us";
double          linkDelay                 = 15e-6;
uint32_t        MachinesPerRack           = 2;
uint32_t        NumberofRacks             = 2;
uint32_t        NumberofSpineSwitches     = 2;
uint32_t        Scale                     = 1;
double          load                      = 0.99;
uint32_t        threshold                 = 100e3;
uint32_t        queueBytes                = 1e6;      // Number of bytes per queue port
uint32_t        initCwnd                  = 10;         // TCP Initial Congestion Window
double          minRto                    = 1000e-6;
uint32_t        segmentSize               = 1460;
bool            LinkUtilization           = 0;
string          StatsFileName             = "9.txt";
double          interval                  = 0.010;

uint32_t        incastDegree              = MachinesPerRack - 1;
double          incastFactor              = 0.5;

double          longLoadFactor            = 0.80;
double          longNodeFactor            = 0.30;

uint32_t        FlowSizeShort             = 16*1024;
uint32_t        FlowSizeLong              = 1024 * 1024;
uint32_t        IncastFlowSizeLong        = 1e9;
bool 		pktspray 		  = 0;
bool		dctcp			  = 1; 
...
Ptr<OutputStreamWrapper> LeafPstream;
...
//  NumberofEndPointsLong  = 0;
//  NumberofEndPointsShort = 0;
 ...
 // shortLoadFactor = (1.0 - longLoadFactor)*(1.0 - incastFactor);
  ...
  NumberofIncastSenders = NumberofEndPointsIncast - 1;//incastDegree * ((int) (NumberofEndPointsIncast/incastDegree));
...
//      NumberofIncastSenders = incastDegree * (((int)(NumberofEndPointsIncast/incastDegree)) - 1 );
  ...
  InterArrival_incast = 0.05; // TODO: Include incast degree in the formula 
...
*/
for (uint32_t i=0;i<NumberofIncastSenders;i++){
    devices = p2p.Install (nEnd[i],nLeaf[i/MachinesPerRack]);
    interfaces = ipv4.Assign(devices);
    ipv4.NewNetwork();
  }

for (uint32_t i=0;i<NumberofIncastAggregators;i++){
    devices = p2p.Install (nEnd[i],nSpine[i/MachinesPerRack]);
    interfaces = ipv4.Assign(devices);
    ipv4.NewNetwork();
  }
...
void SetupClientTraffic(Ptr<Node> Nd,Ptr<Node> ServerNode, uint32_t txsize, uint16_t appPort, Time startTime , uint8_t priority, bool incastServer){
  ...
  source.SetAttribute ("IncastServer", BooleanValue (incastServer));
...
void MeasureLeaf(Ptr<Node> rnode)
{
  Ptr<Ipv4L3Protocol> rnodeip = rnode->GetObject<Ipv4L3Protocol>();
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    dtqueue->SetAttributeFailSafe("LeafSwitch",      BooleanValue(true));
...
void SetupIncastServer(Ptr<Node> rnode)
{
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    ptpnetdev->SetAttributeFailSafe("IncastServer",      BooleanValue(true));

    }

}

void SetupIncastAgg(Ptr<Node> rnode)
{
  Ptr<Ipv4L3Protocol> rnodeip = rnode->GetObject<Ipv4L3Protocol>();
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    ptpnetdev->SetAttributeFailSafe("IncastAgg",      BooleanValue(true));
...
//	}
  	...
  	for (uint32_t i=0;i<AppIdxShort.size(); i=i+2){
		randomShort();
        	...
        	Map_Port[ServerNodeIdx].push_back(appPort);
      		...
      		SetupServerTraffic(nEnd[ServerNodeIdx],appPort,Curr_Start,1, false);
      		SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeShort,appPort,Curr_Start,1, false);
		NumShortFlows++;
                Curr_Start = Prev_Start[ServerNodeIdx] + Seconds (DelayRandomlyShort->GetValue());
                Prev_Start[ServerNodeIdx] = Curr_Start;
                SetupServerTraffic(nEnd[ClientNodeIdx],appPort,Curr_Start,1, false);
                SetupClientTraffic(nEnd[ServerNodeIdx],nEnd[ClientNodeIdx],FlowSizeShort,appPort,Curr_Start,1, false);
		NumShortFlows++;
	}
  }
  ...
  for (uint32_t k=0;k < LongRequestsPerNode; k++){
        ...
        for (uint32_t i=0;i<AppIdxLong.size(); i=i+2){
                ...
                Prev_Start[ClientNodeIdx] = Curr_Start;
                SetupServerTraffic(nEnd[ServerNodeIdx],appPort,Curr_Start,1, false);
                SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],FlowSizeLong,appPort,Curr_Start,1, false);
		NumLongFlows++;
                Curr_Start = Prev_Start[ServerNodeIdx] + Seconds (DelayRandomlyLong->GetValue());
                Prev_Start[ServerNodeIdx] = Curr_Start;
                SetupServerTraffic(nEnd[ClientNodeIdx],appPort,Curr_Start,1, false);
                SetupClientTraffic(nEnd[ServerNodeIdx],nEnd[ClientNodeIdx],FlowSizeLong,appPort,Curr_Start,1, false);
		NumLongFlows++;
        }
  }
...
// random_shuffle(AppIdxIncast.begin(),AppIdxIncast.end()); // commented out on 10/29/2018 for bcube_leaf_spine combination purposes
...
//	SetupIncastAgg(nEnd[AppIdxIncast[i]]);
...
//        for(uint32_t j=0;j<41;j++)
  ...
  for (uint32_t k=0;k < IncastRequestsPerNode; k++){
		...
		randomShort();
                ...
                Prev_Start[ServerNodeIdx] = Curr_Start;
                SetupServerTraffic(nEnd[ServerNodeIdx],appPort,Curr_Start,1,true);
		vector<uint32_t> ClientIdx = Map_Client[ServerNodeIdx];
		for(uint32_t j=0;j<ClientIdx.size();j++)
		{
			ClientNodeIdx = ClientIdx[j];
//			if (j==0 || j==1 || j==2){
//			SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],IncastFlowSizeLong,appPort,Curr_Start,1, true);

//					 }
//			else {
                	
			SetupClientTraffic(nEnd[ClientNodeIdx],nEnd[ServerNodeIdx],IncastFlowSizeLong,appPort,Curr_Start+Seconds (AppDelay()/1000000),1, true);
//				}
		}
        	NumIncastFlows++;
...
/*void GetQueue(Ptr<SimplePriorityEcnQueue> q)
{
for (uint32_t i=0;i<NumberofRacks;i++)
  {
...
//        if(dtqueue->Getqueuesize()!=0){
	     ...
	     // std::cout<<dtqueue->m_bytesInQueue<<"\n";
        }
      }
    }

  }
  Simulator::Schedule(Seconds(0.000010),&GetQueue,q);

}
*/


/*void SetupLeafSwitch(Ptr<Node> rnode)
{
  ...
  for(uint32_t i = 0; i < rnodeip->GetNInterfaces(); i++) {
    ...
    Ptr<Queue> queue = ptpnetdev->GetQueue();
    Ptr<SimplePriorityEcnQueue> dtqueue = queue->GetObject<SimplePriorityEcnQueue>();
    Simulator::Schedule(Seconds(0.000010),&GetQueue,dtqueue);
    }
...
void Setup_Switch_Hooks()
{
  AsciiTraceHelper asciiTraceHelper;
  std::ostringstream ossl;
  ossl << "Leaf." <<StatsFileName ;
  LeafPstream = asciiTraceHelper.CreateFileStream (ossl.str ());
  for (uint32_t i=0;i<NumberofRacks;i++)
  {
   // SetupLeafSwitch(nLeaf[i]);
  }
}
