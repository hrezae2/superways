...
void Build_Topology()
{
  ...
  for (uint32_t i=1;i<NumberofEndPoints;i++){
    devices = p2p.Install (nEnd[i],nLeaf[i/MachinesPerRack]);
    interfaces = ipv4.Assign(devices);
    ipv4.NewNetwork();
  }

//connect all machines in the first rack to spine switches
...
void Setup_Workload(){
  ...
  uint32_t min = MachinesPerRack;
  uint32_t max = 2*MachinesPerRack-1;
  uint32_t appNode = 0.0;
  for(uint32_t i=1;i<NumberofRacks;i++)
  {
  ...
  for (uint32_t i=20;i<NumberofEndPoints;i++)
  {
    bool IncastId = true;
