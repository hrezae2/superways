...
//Type-1 Subways requires connecting switches to multiple ToR switches. 
  for (uint32_t i=0;i<NumberofEndPoints;i++){
    if (i < ((MachinesPerRack * NumberofRacks)  - MachinesPerRack)){
        devices = e2t.Install (nEnd[i],nLeaf[(i+MachinesPerRack)/MachinesPerRack]);
    }
    else 
	devices = e2t.Install (nEnd[i],nLeaf[0]);
    interfaces = ipv4.Assign(devices);
    ipv4.NewNetwork();
  }
