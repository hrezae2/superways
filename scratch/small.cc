...
uint32_t        MachinesPerRack           = 100;
uint32_t        NumberofRacks             = 1;
uint32_t        NumberofSpineSwitches     = 0;
uint32_t        Scale                     = 1;
double          load                      = 0.99;
...
void Configure_Simulator(){
 ...
 // NumberofEndPointsShort = (NumberofEndPoints - NumberofEndPointsLong)*(1.0-incastFactor);
  NumberofEndPointsIncast = NumberofEndPoints;
 ...
 // shortLoadFactor = (1.0 - longLoadFactor)*(1.0 - incastFactor);
  ...
  NumberofIncastSenders = MachinesPerRack - 1;//incastDegree * ((int) (NumberofEndPointsIncast/incastDegree));
...
//        if (i == 12){
  //              if (j == 8 ){
      if(dtqueue->Getqueuesize()!=0){
	      ...
	      std::cout<<dtqueue->m_bytesInQueue << "\t" <<i<<"\t" <<j<<"\t" <<"\n";
//        }
      }
    }
