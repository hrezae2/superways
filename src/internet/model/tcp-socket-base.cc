                            ...
                            Ptr<Ipv4Interface> incomingInterface)
{
  ...
  m_Rdegree = 120;
   ...
   m_node->m_alpha = (4*(m_node->m_alpha)/5) + (m_Rdegree/5);
   m_Rdegree = max(m_node->m_alpha,m_Rdegree); 
   m_Rdegree = m_node->m_alpha;
   //std::cout<<m_node->GetId()<<"\t"<<m_Rdegree<<"\n";
  ...
  //adding reaction to ECN marked packets. Incast aggrigator does this kind of measurments.
  if ((header.GetEcn() == Ipv4Header::CE) && (m_node->m_ecnmarked == false)){
   m_node->m_ecnmarked = true;
   if (m_node->m_addedvalue.size() == 0){
    m_node->m_addedvalue.push_back(2);
        }
		}
  else if ((header.GetEcn() == Ipv4Header::CE) && (m_node->m_ecnmarked == true)){
    m_node->newvalue = ((m_node->m_addedvalue.back()) * 2);
    if (m_node->newvalue < 17)  
    {
    m_node->m_addedvalue.push_back(m_node->newvalue);
    }
    else {
	m_node->m_addedvalue.back() = 16;
	}
  }
  else {
    if (!(m_node->m_addedvalue.size() == 0)){
     m_node->m_addedvalue.back() -= 1;
     m_node->oldvalue = m_node->m_addedvalue.back() - 1;
     if (m_node->oldvalue == 0){
	   m_node->m_addedvalue.clear();
      	   m_node->m_ecnmarked = false;
				}
     if ( (std::find(m_node->m_addedvalue.begin(), m_node->m_addedvalue.end(), m_node->oldvalue)) != m_node->m_addedvalue.end())  {
     // std::cout<<m_node->m_addedvalue.back()<<"\n";
      m_node->m_addedvalue.clear();
      m_node->m_ecnmarked = false;
       }
...
//Adding Incast in Order
  OrderTag otag;
  //m_node->m_incastCount = 32;
 // std::cout<<m_node->m_incastCount<<"\n";
  if (m_node->m_ecnmarked == false){
  otag.SetOrder (m_node->m_incastCount);
  }
  else {
  otag.SetOrder (m_node->m_incastCount * m_node->m_addedvalue.back());
  }
  p->AddPacketTag (otag);
