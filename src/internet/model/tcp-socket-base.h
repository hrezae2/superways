...
uint32_t GetTupleValue (Ipv4Address saddr, Ipv4Address daddr,uint16_t port, TcpHeader tcpHeader);
 ...
 // bool m_ecnmarked;
  ...
  virtual void SendPacket (Ptr<Packet>, const TcpHeader &,
                   Address, Address, Ptr<NetDevice> oif = 0);
...
protected:
  ...
  double	    m_seqnum;
  double	    m_windownum;
  bool              m_blocking;        //< If the socket blocks (for qbb)
  bool		    m_multiqueue;      // If MultiQueue is used
 ...
 // uint32_t oldvalue;
  ...
  // Calculate the ECN marked packets
  uint64_t	    m_totalmarkedPackets;
  uint64_t	    m_totalEcnCount;
